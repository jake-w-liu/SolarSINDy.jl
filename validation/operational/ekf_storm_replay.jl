# ekf_storm_replay.jl — head-to-head storm-time replay: constrained-EKF 1 h forecast vs V2.
#
# Purpose: build the storm-time promotion evidence for the constrained-EKF shadow (the v3 candidate). The
# locked-live record is quiet-only, and the EKF only differs from v2 during storms, so we score the EKF's
# 1 h forecast against V2 (and persistence) on the SAME named storms as storm_replay.jl. The
...
# the deployed _shadow_forecast / constrained _new_filter (decay cap -0.048, Q_COEFF=1e-4) and the locked
# replay_recent_table engine for v2 — no model re-implementation.
#
...
# [DECAY_CAP-2, DECAY_CAP]. (The EKF is a 1 h overlay; multi-step is validated neutral elsewhere.)
#
...
include(joinpath(@__DIR__, "ekf_shadow.jl"))   # _shadow_library, _new_filter, _shadow_forecast, DECAY_CAP, BOOT_YEAR0
...
const MAIN_RATE = -15.0          # dDst/dt below this (nT/h) = rapid main phase; the only regime where v2>persistence
...
repeats the rollout with the FIXED discovered decay; ekf_fixed≈v2 at every lead is the fairness oracle that
the multi-step rollout matches the locked engine, so any ekf−v2 gap is purely the decay adaptation. `rate`
...
function replay_ekf_storm(storm, lib, ξ0, i_decay, cal, f)
    ...
    for it in issues
        ...
        for r in eachrow(g)
            ...
            _, ekf_a = _shadow_forecast(lib, ξ0, i_decay, decay,            anchor_star, drv, latest, cal; nsteps = h)
            _, ekf_f = _shadow_forecast(lib, ξ0, i_decay, ξ0[i_decay],      anchor_star, drv, latest, cal; nsteps = h)
...
and the fairness gap max|ekf_fixed − v2| (should be ≈0 for the multi-step rollout to be a fair comparison)."
function _cell(rows)
    nrow(rows) == 0 && return nothing
    ev2 = rows.obs .- rows.v2; eekf = rows.obs .- rows.ekf; ep = rows.obs .- rows.persistence
    rv2, re, rp = _rmse(ev2), _rmse(eekf), _rmse(ep)
    strong_pers = rp <= rv2                 # stronger baseline = lower RMSE
    Δ, lo, hi = paired_improvement(strong_pers ? ep : ev2, eekf; storms = rows.storm)   # EKF vs the stronger baseline (storm-cluster CI)
    return (n = nrow(rows), rmse_v2 = rv2, rmse_ekf = re, rmse_pers = rp,
            stronger = strong_pers ? "pers" : "v2", improve = Δ, ci_lo = lo, ci_hi = hi,
            fair = maximum(abs.(rows.ekf_fixed .- rows.v2)),
...
function main()
    println("EKF storm-replay: constrained-EKF vs V2 + persistence, leads ", LEADS,
    ...
    open(OUT_MD, "w") do io
        println(io, "# EKF storm-replay — constrained-EKF vs V2 + persistence (multi-lead, causal)\n")
        println(io, "Causal head-to-head on ", length(STORMS), " G4/G5 storms (2023–2025). The constrained EKF ",
                    ...
                    "discovered ODE forward. `improve` is the paired RMSE(stronger baseline) − RMSE(EKF) (positive ",
                    "= EKF beats the stronger of {v2, persistence}); the v3 bar needs its storm-cluster 95% CI strictly > 0. ",
                    "`fair` = max|ekf_fixed − v2|: with the discovered decay held fixed the EKF rollout must ",
                    "reproduce the locked v2 engine, so fair≈0 confirms the multi-step comparison is valid.\n")
        println(io, "| lead [h] | regime | n | RMSE v2 | RMSE EKF | RMSE pers | stronger | improve vs stronger [nT] (95% CI) | fair |")
        println(io, "|---|---|---|---|---|---|---|---|---|")
        println("\n  lead regime    n   v2    EKF   pers  strong  improve[CI]            fair")
        for h in LEADS
            ...
            for (label, sub) in (("pooled", sub_h),
                ...
                @printf("  %3d  %-6s %4d %5.1f %5.1f %5.1f  %-4s  %+5.2f [%+5.2f,%+5.2f]  %.2f\n",
                        h, label, c.n, c.rmse_v2, c.rmse_ekf, c.rmse_pers, c.stronger,
        ...
        verdict = !stable ? "NOT PROMOTABLE: adapted decay left the stable band [$(DECAY_CAP-2), $(DECAY_CAP)]" :
                  max_fair > 2.0 ? "INCONCLUSIVE at multi-step: fairness gap max|ekf_fixed−v2|=$(round(max_fair;digits=2)) nT — rollout does not match the locked engine; trust only fair≈0 leads" :
                  promotable ? "PROMOTABLE: $detail" :
                  "NOT PROMOTABLE: at no lead or regime does the EKF beat the stronger of {v2, persistence} with a 95% CI above 0; the constrained-EKF overlay should remain in shadow"
...
function _selftest()
    ...
    _, ekf = _shadow_forecast(lib, ξ0, i_decay, current_coeffs(f)[1], -20.0, drv, -18.0, cal)
    @assert isfinite(ekf) && -2000 < ekf < 100 "EKF forecast non-finite or out of physical range: $ekf"
    # Oracle 3: under QUIET conditions (Dst*≈0, the decay term ~0) the EKF ≈ v2 within ~5 nT — the gain is
    # storm-time, so it must not manufacture a large quiet-time difference vs the v2 correction baseline.
    v2corr = let feat_df = DataFrame(latest_dst_nt=[2.0], V_kms=[400.0], Bz_nt=[0.0], By_nt=[0.0], n_cm3=[5.0], Pdyn_npa=[1.5])
        ...
        SolarSINDy.operational_v2_correction(cal, feats)
    end
    _, ekf_q = _shadow_forecast(lib, ξ0, i_decay, current_coeffs(f)[1], 2.0, (V=400.0,Bz=0.0,By=0.0,n=5.0,Pdyn=1.5), 2.0, cal)
    # ekf_q is pred_dst + v2 correction; pred_dst for a near-zero anchor with no southward driving ≈ anchor.
    @assert isfinite(ekf_q) "quiet EKF forecast non-finite"
    ...
    @assert _shadow_forecast(lib, ξ0, i_decay, -0.1, -50.0, qdrv, -48.0, cal) ==
            _shadow_forecast(lib, ξ0, i_decay, -0.1, -50.0, qdrv, -48.0, cal; nsteps=1) "nsteps default != 1-step"
    ...
    v1_1 = _shadow_forecast(lib, ξ0, i_decay, -0.1, -100.0, rdrv, -100.0, cal; nsteps=1)[1]
    v1_3 = _shadow_forecast(lib, ξ0, i_decay, -0.1, -100.0, rdrv, -100.0, cal; nsteps=3)[1]
