...
#
# Finding (run below): adaptation does NOT help and can hurt — on May 2024 the 1-step RMSE is
# ~23.3 nT with fixed coefficients vs ~27.8 (adapt injection) / ~32.8 (adapt injection+decay).
# The slow random-walk coefficient drift adds noise rather than tracking a useful signal at
# 1–3 h horizons, and the operational v2 correction layer already absorbs the adaptable drift.
# Decision: the EKF (assimilation.jl) is kept available and correctness-tested, but is NOT wired
# into the live operational path, because the evidence shows it would degrade the forecast.
