...
[ -d "$CLONE_DIR" ] || {
  echo "error: CLONE_DIR is not a directory: $CLONE_DIR" >&2
  exit 1
}
...
SOLARSINDY_V2_CALIBRATION LIVE_MONITOR_INTERVAL_SEC LIVE_MONITOR_PHASE_SAMPLING
LIVE_MONITOR_DEADMAN_CYCLES
...
if [ -n "${LIVE_MONITOR_PHASE_SAMPLING+x}" ]; then
  case "$LIVE_MONITOR_PHASE_SAMPLING" in
    0|1) ;;
    *)
      echo "error: LIVE_MONITOR_PHASE_SAMPLING must be 0 or 1, got '$LIVE_MONITOR_PHASE_SAMPLING'" >&2
      exit 1
      ;;
  esac
fi
